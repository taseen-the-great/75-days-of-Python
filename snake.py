...
#game window
window = tkinter.Tk()  #Tk not TK (small k only, big K not allowed)
window.title("Snake")
...
def game():
    ...
    def move():
        ...
        #update snake body (grows by eating)
    ...
    def draw(): 
        ...
        for tile in snake_body:
            canvas.create_rectangle(tile.x, tile.y, tile.x + TILE_SIZE, tile.y + TILE_SIZE, fill = "brown")

        if (game_over):
            canvas.create_text(WINDOW_WIDTH/2, WINDOW_HEIGHT/2, font = "Calibri 20", text = f"GAME OVER: {score}", fill = "white")
        else:
